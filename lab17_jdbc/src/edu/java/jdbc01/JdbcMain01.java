 ...
 * 5. Statement 객체 생성 - 데이터베[이스 서버에서 실행할 SQL 문장 작성, 실행
 * 6. Statement 객체의 메서드를 사용해서 SQL 문장을 데이터베이스 서버로 전송하고, 그 결과를 받음.
 *  (1) exccuteQuery() - select
...
public class JdbcMain01 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            stmt = conn.prepareStatement(sql); // 연결이라는 객체에서 메소드 호출하는 것.
            
            // 6. SQL 문장을 DB 서로 전송, 결과 받음.
            rs = stmt.executeQuery();
            System.out.println(rs);
            
            // 7. SQL 실행 결과를 화면에 출력.
            while (rs.next()) { // ResultSet에 행(row)
            ...
            // 8. 사용했었던 리소스들을 해제(close) - 객체들이 생성된 반대 순서로 close.
